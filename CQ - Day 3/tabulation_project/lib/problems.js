...
function maxNonAdjacentSum(nums) {
    ...
    for(let i = 1; i < nums.length; i++){
        if (i === 1){
            if (nums[i] > nums[i-1]) table[i] = nums[i];
            else table[i] = nums[i - 1];
        } else {
            let numbers = nums[i] + table[i - 2];
            if (numbers > table[i-1]) table[i] = numbers;
