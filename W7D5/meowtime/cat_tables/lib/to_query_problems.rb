...
def no_apples_for_blair_sub
  ...
  execute(<<-SQL)
    ...
    FROM
      cats
    INNER JOIN cattoys ON cattoys.cat_id = cats.id
    WHERE
      cattoys.toy_id IN (
        SELECT 
          toys.id
...
def price_like_shiny_mouse
  ...
  execute(<<-SQL) 
    ...
    FROM
      toys AS a
    INNER JOIN
      toys AS b on a.price = b.price
    WHERE 
      a.name = 'Shiny Mouse' AND b.name != 'Shiny Mouse'
    ORDER BY 
      b.name;
  SQL
end
...
def just_like_orange
  ...
  FROM
    cats a
    JOIN cats b ON a.breed = b.breed
  WHERE
    a.name = 'Orange' AND b.name != 'Orange'
  ORDER BY
  b.name;
  SQL
end
...
def toys_that_jet_owns
  ...
  FROM
    ...
    JOIN cattoys as c2 ON c2.toy_id = t1.id
    JOIN cats a2 ON a2.id = c2.cat_id
  WHERE
    a1.name = 'Jet' AND a2.name != 'Jet'
  ORDER BY
    a2.name;
  SQL
end
...
# JOIN
  ...
  # JOIN 
  #   cats a ON a.id = c1.cat_id
  # JOIN
  #   cattoys c2 ON c2.cat_id = a.id
  # JOIN
  #   toys  t1 ON t1.id = c2.toy_id
  # JOIN 
  #   cattoys c3 ON c3.toy_id = t1.id
  # JOIN
  #   cats b ON c3.cat_id = b.id
